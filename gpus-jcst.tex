...
\author{P. Garc{\'\i}a-S{\'a}nchez$^1$, A.M. Mora$^2$,Gustavo Romero$^3$\\
  J. J. Merelo$^1$, P.A. Castillo$^3$, M.G. Arenas$^3$, Antonio Fern�ndez-Ares$^2$  \\ \\
...
% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
% ANTONIO:  Please everyone to be aware of this!!!
%
% (according to the literature)
%
% GPU -> Graphics Processing Unit  (physical hardware)
%
% GPGPU -> General Purpose computing on a GPU (software adapted to run on a GPU)
%
% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
\end{abstract}
 ...
 \begin{keyword}
parallel computation \sep evolutionary algorithms \sep metaheuristics \sep GPU \sep GPGPU \sep survey
\end{keyword}
%
%\end{frontmatter}
...
GPUs (Graphics Processing Unit) specialized stream processors, composed of hundreds (or thousands) of computation units, and designed to perform graphics manipulations applying optimized graphical primitive operations, at a much higher speed than a general purpose CPU.
However, they have been adapted to be used either as general purpose multi-processors with shared memory, or as a Single Instruction Multiple Data (SIMD) processors \cite{Pharr12SPMD}; their utilization as general purpose processors is usually called GPGPU \cite{luebke2006gpgpu,owens2007}. 
Their use in all kind of computers inspired the development of 
specific programming languages, tools and applications that leverage the power of the architecture, favouring the emergence of specific solutions, frameworks and algorithms that take advantage of those devices. Besides from our
paper in 2012 \cite{arenas2012gpu}, there are many new ways of using
GPUs for general purposes, from cloud instances that include them to
{\em desktop supercomputers} that boast several GPU linked by a
high-performance network.

These advances have extended the use of GPGPU to many other labs and
...
Evolutionary Algorithms (EAS) \cite{de2016evolutionary}, has been the
application target of GPGPU in many occasions. EAs are a biologically
...
Finally, some of the new trends and open challenges in the scope of GPGPU are discussed.
...
utility for GPGPU are described, as well as a comparison of the
...
power of GPGPU (Section \ref{sec:parallelizing}). In the same
...
On the other hand, since a GPU is a kind of stable and reliable architecture, we consider the classification also as valid for the purposes of this paper. We will present these parallelization modes below.
...
using GPUs for General Purpose computing (GPGPU). These aspects normally
...
implementation of any other kind of algorithm (for GPGPU), such as graph
...
General Purpose computing on Graphic Processing Units (GPGPU) to implement these
approaches.
