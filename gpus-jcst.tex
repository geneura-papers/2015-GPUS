...
\begin{abstract}
  ...
  % meaningless. - JJ
Evolutionary Algorithms (EAs) are the most prolific metaheuristic
adapted to General Purpose Graphics Processing Units (GPGPUs), as they
are inherently parallel population-based approaches.
% It's not a tutorial. It's a survey. Do you want to show people how
% to put them to use? - JJ
Thus, this paper firstly aims to be a tutorial regarding the
parallelization of EAs on GPGPUs, introducing their general features
and main distributed models, together with several frameworks which
allow to conduct this task easily, and the main issues to take into
account in order to carry out these parallel implementations.
...
the adaptation of EAs to GPGPUs. Thus, 194 publications have been
reviewed, extracting their key features to categorize them. % Better
                                % not to say a number, because it will
                                % change from version to version - JJ
We have followed a taxonomy of the approaches used for GPGPU
parallelization. % Followed which taxonomy? You mean followed with? - JJ
Then, a more in-depth analysis has been conducted on
the most representative works in each category. % In depth analysis of
                                % what? - JJ
\end{abstract}
