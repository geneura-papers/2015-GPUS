...
The most complete and comprehensive review of these kind of frameworks was proposed by Parejo et al. \cite{springerlink:10.1007/s00500-011-0754-8}. It is a comparative study of metaheuristic optimization frameworks, and therefore it is not focused only on EAs. 6 different areas and characteristics were studied using 271 features. Between these features, the differences between solution encoding or parallel and distribution mechamisms were compared, but also other software engineering issues, such as the documentation or integration, were studied. However, none of the ten most extended optimization frameworks, ECJ, Eva2, FOM, ParadisEO, jCLEC, Opt4j, OAT, EasyLocal, HeuristicLab and MALLBA, included references to GPU capabilities when this paper was published.
...
the Single Program Multiple Data (SPMD)\cite{Pharr12SPMD} programming
...
The impact of Cellular Evolutionary Algorithms (CEAs) or Fine-grained Evolutionary
Algorithms has been smaller with respect to other types of PEAs. The
main reason is that they are better suited for shared memory architectures than the ones with distributed memory, but also due to the high cost of building this type of architectures.

However, a review of the trends in parallel
architectures foresees a strong increase in fine-grained EAs face to
face other EA categories. This can be explained for the next three reasons:

\begin{itemize}
\item The amount of processors on a chip or card as massively grown.
\item Fine-grained EAs efficiency has been increased thanks to the high-processor speed.
\item A large number of researchers have been attracted by the low cost of these architectures.
\end{itemize}

In this kind of algorithms, a neighborhood-based selection is performed to select the mating parents, thus providing automatic niching effect and avoiding early convergence.
