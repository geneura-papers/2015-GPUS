...
\begin{abstract}
  % We should avoid qualifiers such as "most prolific". They are
  % meaningless. - JJ
% Antonio - done
Evolutionary Algorithms (EAs) have been widely adapted to General Purpose Graphics Processing Units (GPGPUs), as they are inherently parallel population-based approaches.
% It's not a tutorial. It's a survey. Do you want to show people how
% to put them to use? - JJ
% Antonio - Yes, there is a section about it and one of the journals aimed for tutorials. I have adapted it.
Thus, this paper aims firstly to describe the parallelization process of EAs on GPGPUs, introducing their general features and main distributed models, together with several frameworks which allow to conduct this task easily, and the main issues to take into account in order to carry out these parallel implementations.
% Does it? Since 2015? - JJ
% Antonio - we are updating it to 2018, so...yes
The work also reviews the new advances in hardware and software in this field, as well as new trends and challenges arising in this scope.
Finally, the paper presents a deep survey on the literature related to
the adaptation of EAs to GPGPUs. Thus, more than 200 publications have been
reviewed, extracting their key features to categorize them. 
% Better not to say a number, because it will change from version to version - JJ
% Antonio - fixed
The papers have been grouped following a taxonomy of the approaches used for GPGPU parallelization. 
% Followed which taxonomy? You mean followed with? - JJ
% Antonio - Now it is clearer I think
Then, a more in-depth review has been conducted on the most representative works in each category, extracting their main features and findings, and some statistical analyses on the publication history and tendency have been conducted.
% In depth analysis of what? - JJ
% Antonio - It is better now
Finally, we have conducted a comparative analysis with other high-performance computing methods, such as parallelization through Cloud Computing or the use of FPGAs for accelerating specific operations.
\end{abstract}
...
The main features common to all the approaches are a population of
individuals or potential solutions for the target problem, a selection
method that favours better solutions, and a set of operators that
iteratively act upon the selected solutions. If there is defined a
correct {\em fitness function}, which assigns a reliable value to
every individual, this process guarantees that the average quality of
...
Even if EAs are sequential algorithms, their features, such as the
population of independent solutions they manage, or some
operators/functions which are applied on every individual at a time,
...
Next, Section \ref{sec:taxonomy} proposes a taxonomy based on the different parallel models implemented in the literature, in order to categorize the reviewed papers. Thus, in Section \ref{sec:survey} a set of representative works (according to their number of citations) describing different approaches using GPGPUs are reviewed and divided following the proposed taxonomy, plotting also some interesting numbers and graphs related to them.
GPGPU computing is compared with other high-performance approaches in Section \ref{sec:comparison-hpc} and new trends and challenges are discussed in Section \ref{sec:discussion}. Finally, Section \ref{sec:conclusions} remarks reached conclusions.
...
% Antonio - I think we don't need to justify this
...
At the same time graphic cards became general-purpose devices long time ago. Indeed, earliest academic work about general computation on GPUs date back to 2002 at the University of Washington \cite{Thompson:2002:UMG:774861.774894}, and 2004 in Stanford \cite{Buck:2004:BGS:1015706.1015800}. 
% so... - JJ
% Antonio - rewritten
...
% Antonio - Not really. I remove it
...
instance, basic cores %which are ... - JJ  % Antonio - it is said in the following sentence
...
This section introduces the most relevant aspects or concepts to know before using GPUs for General Purpose computing, which normally differs between vendors, namely: the different programming possibilities, the execution model that GPU computing follows, and the memory scheme to use.
...
fusing the master-slave evaluation with the diversity mechanisms of
island-based models. 
...
\item Real World Problems: Real problems usually imply to deal with massive amount of records and high-dimensional data. In fact, new techniques proposed also address other related issues such as filtering, noise-removal or feature selection. 

\item Big data: Dealing with all these previous issues has led to the emergence of new techniques, mainly the Map-Reduce paradigm. Different authors propose to...  DeepLearning \cite{Gadea-Girones2018} 
...
\item Multi-objective optimization: Dealing with more than one optimization objective implies a different way to deal with the fitness calculation. As all the solutions need to be compared to each other to calculate their Pareto Front, traditional parallel schemes need to be adapted in a non-trivial way...

\item Parameter control: Since the origin of the research 



\item Heterogeneous architectures: Different methods to combine technologies ... have been proposed. Even as dozens of low-cost Multi-CPU-FPGA can achieve the same performance than a GPGPU, the GPGPUs are still the best cost-effective solutions \cite{Gadea-Girones2018}, so finding techniques to efficiently combine both technologies is the key to outperforms its inherent limitations. However, a lot of works do not include enough details about the benefits to maximize the usage of CPU-GPU combinations to take leverage the irregularity of data access. Future trends will include new methods related to the improvement of core and cache efficiencies \cite{Escobar17GPUCPU}. 

\item Load balancing: Related with the previous issue, different master-slave approaches have been proposed, usually distributing the evaluation of the individuals among the available threads in the GPU. However, new trends involve island-based models...

\item Energy consumption: Techniques such as power-aware scheduling \cite{Escobar17GPUCPU} are being proposed to deal with ...
