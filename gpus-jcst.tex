...
\author{P. Garc{\'\i}a-S{\'a}nchez$^1$, A.M. Mora$^2$,Gustavo Romero$^3$\\
  J. J. Merelo$^1$, P.A. Castillo$^3$, M.G. Arenas Antonio$^3$, Fern�ndez-Ares$^2$  \\ \\
...
\begin{abstract}
Evolutionary Algorithms (EAs), a population based stochastic approach to problem solving, 
have been widely adapted to General Purpose Graphics Processing Units (GPGPUs), 
using the fact that the different solutions in the population can be evaluated in
...
have been adapted to be used either as general purpose multi-processors with shared memory, 
or as a Single Instruction Multiple Data (SIMD) processors
\cite{Pharr12SPMD}; their utilization as general purpose processors
is usually called GPGPU \cite{luebke2006gpgpu,owens2007}. 
Their use in all kind of computers inspired the development of 
specific programming languages, tools and applications that leverage the power of the architecture, 
favouring the emergence of specific solutions, frameworks and
algorithms that take advantage of those devices. Besides from our
...
These advances have extended the use of GPGPUs to many other labs and
...
application target of GPGPUs in many ocassions. EAs are a biologically
...
the procedure is repeated until the solution, or a suitable one is found. 
...
frameworks which can help to do this task. After this introduction, the work gives some guidelines to parallelize an EA using GPUs, pointing out the main issues and constraints that should be taken into account.
