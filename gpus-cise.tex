...
The application of EAs to find the solution of either real world problems or large instances of some benchmark problems, requires a high amount of computational power. Thus, it is quite frequent to implement a parallel or distributed version of this kind of algorithms, aiming to reduce the required computational time to solve the problem. Moreover, sometimes these parallel schemes also lead to an improvement on the quality of the obtained solutions.
% What do you mean by "whereas, ...". Please rephrase. 
% Antonio - done

Even if EAs were proposed as sequential methods, their features, such as the population of solutions they manage, or some operators/functions which are applied on every individual at a time, make them intrinsically parallel approaches, and thus, very suitable to be distributed or parallelized.

% This sentence is also too general. Specify the sequence in a few
% words, and then explain how it can be generalized - JJ
% Antonio - rephrased.

In the last years, new emerging trends in parallelization and distribution are arising. These are technologies such as Peer-to-Peer (P2P) \footnote{\url{https://en.wikipedia.org/wiki/Peer-to-peer}}, Cloud Computing \footnote{\url{https://en.wikipedia.org/wiki/Cloud_computing}}, or Volunteer Computing \footnote{\url{https://en.wikipedia.org/wiki/Volunteer_computing}}, to cite the most relevant.
These trends require a new way to develop parallel EAs taking into account some shortcomings, such as integration of heterogeneous elements or dynamic resources, and also to deal with fault-tolerance, churn, massive
scalability or decentralization.
% Antonio - add here a cite?

However, the advances in the video game industry have led to the production of low cost and high-performance graphics processing units (GPUs), which mitigate several of these issues. 
