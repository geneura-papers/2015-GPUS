...
schemes for EAs. 
This paper is mainly devoted to describe them and presents a
survey on the literature related to their adaptation to GPGPUs. 
More than 200 related publications from the last 13 years have
...
on a wide selection, also describing the most representative work in each
category.
But previously, the work revises the new advances in hardware and software in this field, and gives an introduction on how to parallelize EA algorithms on GPGPUs, describing both several frameworks which allow to do it easily, and the main issues to have into account for these implementations. 
\end{abstract}
...
\hline
\hline
AMD Ryzen 7 1800X       & 4.8           & 195          & 8             & 3.6        & 46           & 538.9          & 95  & 465    \\
\hline
...
A Genetic Programming (GP) method proposed by Harding and Banzhaf \cite{4215552} is another example of execution of the EA using the GPU for fitness evaluation, while the rest of the steps are executed on the CPU. In this paper real-coded expressions were tested in up to 10000 nodes, boolean expressions in up to 1500 nodes, and some real world problem in up to 10000 nodes. Results yielded speedup of thousand times in some instances.
...
Hierarchical parallel GAs have been adapted using other models. For example, Zhang et al. \cite{ZhangImplementationMasterSlave}, propose the use of {\em deme} model at high level, and a master-slave schema at low level. In this case, the CPU initializes the populations and distributes the individuals to thread blocks in shared memory. Then, the GPU threads within each block run a GA independently, by applying  selection, crossover, mutation and evaluation, and migrates individuals to other thread blocks in its neighborhood. However, no speedup results were reported in this paper.
