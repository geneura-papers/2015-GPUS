...
The first step for parallelizing an evolutionary algoritm using a GPU, is to study the GPU architecture, because each GPU has special features the programmer has to be in mind to take adventage of it. Due to that reason, the algorithm and so the code must to be adapted as well as possible to the GPU's architecture where the algorithm will be runned. If this basic requirement is not clear for everybody, the adventage the programmer hope to get, can become a disadvantage, because the not arranged parallel version of code might spend more time than the sequencial one. 
...
\item In CPU scope:
	\begin{itemize}
		...
		\item To define the grid of threads for kernel function execution, fixing the dimmension of each thread's block and, how many and how the blocks are organised for the grid using one, two or three dimmension. 
