...
\item In GPU scope:
	\begin{itemize}
		...
		\item Allocate memory for the data the individuals need for their evaluation (e.g. the matrix distances for a Traveling Salesman Problem).
...
speedup the next data loading. %loadings? - JJ
...
The survey in Alba \cite{Alba2005book} reviewed parallel metaheuristics
on EC. Most paradigms were identified with respect to
...
The parallel EAs community has a wide legacy % wide legacy is not a
                                % thing - JJ
with MIMD architectures and a very little contribution for SIMD systems. The MIMD dominance over SIMD seems to be responsible for this.
