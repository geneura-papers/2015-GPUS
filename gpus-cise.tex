...
from the last 13 years have been revised, and some properties have been extracted: type of EA
...
Depending on the power of a GPU, the number of threads that can be run in parallel on such devices is currently in the order of hundreds to thousands and this number is expected to increase every year. Nowadays, developers can write their own high-level programs to be run on GPU. Due to the wide availability, programmability, and high-performance of these consumer-level GPUs, they are cost-effective for, not just game playing, but also scientific computing.
...
Thus, this paper presents a survey of the most extended EAs and the different proposals of GPUs-parallelization approaches made in a number of research works. They are organized according to a parallelization taxonomy, which is also introduced.
...
Section \ref{sec:eas} presents GPUs as highly parallel devices architectures. 
In Section \ref{sec:parall_and_GPUs}, a review of different general purpose GPUs is presented, giving details on different hardware products.
Then, a background on the different higher level programming languages used to take advantage on GPUs is described (Section \ref{sec:programming}) followed by a description of the most frequent frameworks used to parallelize EAs to take advantage of the power of GPGPUs in Section \ref{sec:parallelizing}. 
Next, Section \ref{sec:taxonomy} proposes a taxonomy based on the different parallel structures implemented in the literature, in order to categorize all the papers.
Then, in Section \ref{sec:survey} a set of representative works (according to their number of citations) describing different approaches using GPUs are reviewed and divided following the proposed taxonomy. 
...
Moreover, there are other parallelization approaches where the GPU runs not only the evaluation stage, but all the stages of the algorithm. If the developer decides to use this option, usually each thread, or each block of threads, evolve few individuals, and all thread blocks cooperate for population evolution. This second approach is easily implemented when the evolutionary algorithm is a fine-grained one. If you, as programmer, decides to parallel using this approach, each thread of the block usually evolves one individual or part of one individual and interacts with neighbour threads for the rest of algorithm stages. 

	
At the end, the challenge for using a GPU is to understand the Single Program Multiple Data programming method and to take advantage of it for your algorithm. The  
implementation's requirements will depend of the problem nature and of the programmer ability. We have tried to make a list, but all the points of this list could get over with innovation and experience. 

In the other hand, the constraints could be related with the problem size,  with the memory consumption, or with the threads coordination, but we are sure the experience and the innovative ideas of GPU's coders, will find solutions for each constraint providing new tools and tips for Evolutionary Computation researcher.
