...
much higher speed than a general purpose CPU. % composed by → lo que
                                % hacen los compositores - JJ
...
(SIMD)\footnote{\url{https://en.wikipedia.org/wiki/SIMD}}
architectures. To this end, there are specific programming languages
and structures that can be used by developers to write their own
high-level programs to be run on a GPU, which are cost-effective for
scientific computing, given their extremely high performance (in the
comparison with CPU-based computing). 
...
frameworks which can help to do this task. The work also describes
how % "also" should be avoided. Use some logical particle. "after
    % doing this, we do that which emanates from the first thing" - JJ
to parallelize an EA using GPGPUs, pointing out the main issues and
...
comparison of the existing cards in the market is shown. % Remember:
                                % almost every time you use
                                % "regarding", you should not - JJ
...
the same section, the main issues to consider in order to parallelize
an EA on a GPGPU are commented.  
...
the representation can also be a real-value vector.  % This is
                                % relevant for what? - JJ
...
considered as an independent unit \cite{Alba13parallel}. % That's not
                                % really true... You need several
                                % individuals for most operations - JJ
Thus, in principle, there are several possible ways to parallelize % You can paralellize in several ways even if it is not inherently paralellizable - JJ
them: for example, fitness evaluation can be conducted in several
slave machines, or the population can be distributed among different
nodes to be evolved at the same time.  
...
and the nodes are reliable and trustworthy. % Antonio - TO DO: add a brief description of other parellelization models, such as P2P or Cloud Computing-based ones. We could add our own cites (Juanlu's and Maribel's/JJ's). PABLO: Added
...
The need of moving data in and out of the GPU memory is very common: to start a calculation, to gather results,... If the bandwidth consumed by this process involves an excessive amount of time the target speedup can be ruined. Data movement should be avoided or at least minimized. This bandwidth problem has arose many times in the computation era. Every time a new powerful device has appear we must feed it with enough data. Two solutions of recent appearance try to alleviate it. The first is a fourth level of cache between the CPU and the GPU. This is only useful when both processors are glued together in the same chip or encapsulation. The second solution involves changing the way we access memory from GPU and its called Heterogeneous System Architecture (HSA). HSA shares data between CPU and GPU in a seamless way, avoiding as many memory copies as caches permits. This way both devices can share a common memory architecture.
...
Previous steps can be implemented in any CUDA version. Moreover, in recent versions of this programming language those steps can be simplified, as the GPU processors are able to access CPU memory scope directly.
However, this approach is not useful in most of the cases, as the time required for CPU memory access is too large. Thus, normally, it is only used when the data to access is just needed one time for one operation. If the evaluation process needs to use the data several times, it might be better copying them to the GPU memory, in order to speedup following data loadings.

There are other parallelization approaches where the GPGPU runs not only the evaluation phase, but all the stages of the algorithm. 
...
In that case, the implementation will be based on assigning one individual (or even a part of it) to each thread of a block. Then, related threads must interact with their neighbours in order to `co-evolve' individuals and also to carry out the rest of algorithm stages. 

Finally, it is very important to understand and to properly implement the Single Program Multiple Data (SPMD)\footnote{\url{https://en.wikipedia.org/wiki/SPMD}} programming concept, in order to take advantage of it in the EA implementation. The programmer should always take into account the constraints related to the problem size, to the GPU (and computer) memory consumptions, or to the synchronization of threads, as all these issues might affect the obtained performance.
...
This basic feature of coarse-grained EAs hits a physical limit of GPUs. 
...
However, the GPU architecture is not designed for synchronising different kernels into the same grid.
Thus, taking into account this limitation, using a GPU to run a coarse-grained EA might require changing some standard mechanism of the algorithm.
...
This work has considered the most parallelized and distributed metaheuristic on the literature, Evolutionary Algorithms (EAs), and has conducted an exhaustive revision of the existing works, using General Purpose Graphic Processing Units (GPGPUs) to implement these approaches. 
