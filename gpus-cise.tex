...
\item In GPU scope:
	\begin{itemize}
		...
		\item Each GPU processor evaluates one individual or part of each individual executing one or more kernel. The first one is called from the CPU scope and the rest of them might be called from the same scope or from the GPU one.
	\end{itemize}  
\item In CPU scope:
	\begin{itemize}
