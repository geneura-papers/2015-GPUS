...
\title{Parallelizing Evolutionary Algorithms on GPUs:\\ Current Methodologies and Trends}
...
parallel, based on a population of independent representations of
solutions. Thanks to this feature, there are many different distributed
schemes for EAs. This paper is devoted to describe them and presents a
survey on the literature related to their application on GPGPUs, along
with a review of advances in hardware and software and other new
trends in the field. 
More than 200 associated publications, from the last 13 years, have
been revised and their properties have been extracted: type of EA
implemented, hybridization, configuration and whether they have been
applied to solve real world problems.  
We also define a taxonomy of the approaches used for parallelization
on the literature. Then, a more in-depth analysis has been conducted
on a wide selection, also describing the most representative in each
category. 
...
% [pedro]  I suggest changing this sentence, trying to indicate something like:  main approaches to solve real world problems taking advantage of GPGPUs have been reviewed.
% Antonio - TODO: complete with more conclusions...
% Don't like that "remark" either - JJ
\end{abstract}
...
The application of Evolutionary Algorithms (EAs) to obtain the solution of
...
Initially the EAs were proposed as a sequential set of steps with different operators and data structures. Then, several ways to parallelize these algorithms were presented, with new schemes and operations.  % operators    ???
...
Since the creation of the EAs, many Universities and research groups have developed a number of frameworks to facilitate the creation of this kind of algorithms. Many of them include implementations for the most commons operators, problems and individual representations, using different technologies and programming languages.

The most complete and comprehensive review of these frameworks is from Parejo \cite{springerlink:10.1007/s00500-011-0754-8}. It is a comparative study of metaheuristic optimization frameworks, and therefore not only focused in EAs. As criteria for comparison, a set of 271 features grouped in 30 characteristics and 6 areas were selected. These features include the different metaheuristic techniques covered, mechanisms for solution encoding, constraint handling, neighborhood specification, hybridization, parallel and distributed computation, software engineering best practices, documentation and user interface. However, out of ten most extended optimization frameworks (ECJ, ParadisEO, Eva2, FOM, jCLEC, OAT, Opt4j, EasyLocal, HeuristicLab and MALLBA), none included references to GPU capabilities when this paper was published.

But in the recent years, some of these EA frameworks have included some kind of GPU integration. Some authors have created library extensions to existing frameworks to allow the evaluation of the individuals in GPUs, for example Robilliard et al. \cite{RobilliardECJGPU08} for ECJ, and Cano et al. for jCLEC \cite{CanoJCLECGPU12}. ParadiseEO-MO-GPU \cite{MealbParadiseoGPU13}, is another framework built upon one existent one, but for now, it is only focused in single solution metaheuristics, and not in population-based ones, such as the EAs. 


PUGACE\cite{5586286} is one of the few new frameworks focused exclusively on the execution of EAs in GPUs. It is based on MALLBA and uses CUDA to execute cellular GAs (cGAs), but only for linear neighborhood structures. %The architecture of this framework is presented and experiments with the QAP problem are performed. Fitness evaluation uses a thread for each chromosome. Speedups for several problem instances varies from $\times15$ to $\times18$. The execution platform was a Pentium Dual Core at 2.5Ghz and an nVidia GeForce 98000 GTX+. 
 Finally, EASEA \cite{Maitre:2009:CGP:1569901.1570089} is another attempt to facilitate the development of EAs to be run in GPUs. This is done by compiling from a specific algorithm specification language (EASEA) to code suitable to be run in a CPU-GPU manner, but as in previous frameworks, only the code for the fitness evaluation is sent to the GPU to be executed in parallel.
...
Besides, Chitty et al. \cite{Chitty2016} proposed different techniques to improve the performance of a graphics card implementation of tree-based GP in order to better exploiting this faster device. Authors demonstrated that both L1 cache and shared memory need to be considered for extracting the maximum performance.
