...
problem has arisen many times in the computation era. % computation
                                % era? WTF? - JJ
Every time a new
...
\cite{SpeedingTheEvaluationofGPCano:2012}.  % 2008 is not recently - JJ
...
evaluation is sent to the GPU to be run in parallel. % That is not
                                % good because... JJ
...
speedup the next data loadings. %loadings? - JJ
...
(SPMD)\footnote{\url{https://en.wikipedia.org/wiki/SPMD}} programming
concept, in order to take advantage of it in the EA
implementation. The programmer should always take into account the
constraints related to the problem size, to the GPU (and computer)
memory consumptions, or to the synchronization of threads, as all
these issues might affect the obtained performance.
...
The parallel EAs community has a wide legacy % wide legacy is not a
                                % thing - JJ
...
the previously described taxonomy. % Why not the other? - JJ

\subsection{Paper sampling} % Gustavo: que te parece este
...
be transferred and updated. % avoiding bandwith problems? - JJ
The run is conducted for 50 generations
and with 500 individuals of a maximum depth of 3 levels, using a
i7-3820 CPU and 8 CUDA GPUs (GeForce GTX 690). % You need to give that
                                % amount of details? - JJ
...
not designed for this task. %In fact, the CPU is not either. - JJ
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Coarse-grained Approaches (island model)} % why mix caps
                                % and not? - JJ
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
