...
Internal CPUs parts have well known standard names. With GPUs the situation is the opossite. Every vendor has a different name for the same part. People try to easy this problem by calling {\em shader core} to an Intel Execution Unit (EU), an AMD Single Instruction Multiple Data (SIMD) or an NVIDIA Streaming Multiprocessors (SM). A shader is a processor core inside of a GPU capable of fetching, decoding, issuing and executing instructions. Every shader is composed by several execution units (EU). Every EU can execute an individual vector operation in the same way of an Advanced Vector Extensions (AVX) or Streaming SIMD Extensions (SSE) instruction. AMD prefer the term streaming processors (SP), and NVIDIA, CUDA core. In the next Table, \ref{tab:features}, we can see detailed comparison of these parts inside modern CPUs and GPUs.
...
\hline
Intel Core i7-7700K     & 1.75          & 149          & 4/24          & 4.2/0.950  & 68           & 745.24         & 91  & 339    \\
\hline
\hline
