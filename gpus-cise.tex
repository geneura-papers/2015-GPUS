...
First, we introduce the most general parallelization approach. That uses normal processors, CPUs, for running the evolutionary cycle and the GPU's processors for running the evaluation stage. This approach is the most simple for implementation and the programmer only change a part of their evolutionary algorithm codification. The steps for this approach are carried out in two scopes, the CPU and the GPU one:
\begin{itemize}
\item In GPU scope:
	\begin{itemize}
...
Moreover, there are other GPU's parallelization approaches where the GPU parallizes not only the evaluation stage, but all the stages of the algorithm. 
