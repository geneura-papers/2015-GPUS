...
either real world problems or large instances of some benchmark problems, requires a high amount of computational power. Thus, it is quite frequent to implement a parallel or distributed version of this kind of algorithms, aiming to reduce the required computational time to solve the problem. Moreover, sometimes these parallel schemes also lead to an improvement on the quality of the obtained solutions.
% What do you mean by "whereas, ...". Please rephrase. 
% Antonio - done

Even if EAs were proposed as sequential methods, their features, such as the population of solutions they manage, or some operators/functions which are applied on every individual at a time, make them intrinsically parallel approaches, and thus, very suitable to be distributed/parallelized.

% This sentence is also too general. Specify the sequence in a few
% words, and then explain how it can be generalized - JJ
% Antonio - rephrased.

% Antonio - TO-DO (for me): Continue rewriting the abstract
