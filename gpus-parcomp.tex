...
Alba classifies the main parallel metaheuristic models as follow: % si
                                ...
                                % metaheuristics - JJ
\begin{itemize}
\item Parallel Genetic Algorithms (Cant\'u Paz \citep{Cantu-Paz98asurvey}) %(G. Luque, et al.).
\item Parallel Genetic Programming (F. Fern\'andez, et al.\citep{springerlink:10.1023_A:1021873026259}).
\item Parallel Evolution Strategies (G. Rudolph \citep{Rudolph92parallelapproaches}).
\item Parallel Ant Colony Algorithms (S. Janson, et al. \citep{ParallelAntColony}).
\item Parallel Estimation of Distribution Algorithms (J. Madera, et al. \citep{springerlink:10.1007_3_540_32494_1_7}).
\item Parallel Scatter Search (F. Garcia, et al. \citep{GarciaLopez2003575}).
\item Parallel Variable Neighborhood Search (F. Garc\'ia-l\'opez, et al.\citep{Garcia_lopez_theparallel}).
\item Parallel Simulated Annealing (\citep{Genetic_parallelsimulated}) .
\item Parallel Tabu Search (T. Crainic, et al.\citep{Crainic97towardsa}).
\item Parallel Greedy Randomized Adaptive Search Procedures (M. Resende and C. Ribeiro \citep{Resendeparallelgreedy}).
\item Parallel Hybrid Metaheuristics (C. Cotta, et al. \citep{Cotta05e:parallel}). 
\item Parallel MultiObjective Optimization (A. Nebro, et al. \citep{Nebro07mocell:a}).
\item Parallel Heterogeneous Metaheuristics (F. Luna, et al. \citep{ANL04}). 
\end{itemize}
