...
%TODO mirad si el título está bien
...
% Todo lo de arriba es la motivaciÃ³n del trabajo, no la del paper. DeberÃ­a ir a la intro. En el abstract debÃ©is decir quÃ© se hace en el paper - JJ
% creo que JJ tiene razÃ³n pero a lo mejor lo podemos dejar un poco mÃ¡s cortito - gustavo
% Antonio - lo he cambiado, espero que esté mejor. ;)
\end{abstract}
...
%algoritmos bioinspirados. Es una revista generalista y tendríais que
%hacerlo - JJ
% Antonio - lo justifico debajo
...
EAs present a structure which is inherently parallel, because the individuals of the population are relatively independent (they work independently, but all of them cooperate to find the solution). Thus, their distribution on different computation nodes is quite straight and not very difficult to implement, being normally a node devoted to work on subpopulations and even, in the finest-grained models, every node is assigned to one individual \cite{Alba02Parallelism}.
...
For these reasons, EAs are so far the most prolific metaheuristic in terms of existing parallel approaches, including the distribution on GPUs.
%
% Antonio - explicar un poco mejor e incluir alguna referencia.
%
...
In addition, this work introduces the features and advantages of GPGPUs, then it explains the general approaches to use and profit a GPGPU model, and gives an overview of currently available programming tools and software systems.

The rest of the paper is structured as follows: first we explain GPUs as highly parallel devices architectures. Then, a background on the different
higher level programming languages used to profit GPUs is explained. Next section proposes a taxonomy to differentiate between all the papers, that are reviewed in next section. Finally, the conclusions are presented.  %PABLO: Esta plantilla NO tiene numeros de sección
...
\begin{figure*}[!ht]
\centering
\includegraphics[width=16cm]{cpu-gpu.png}
...
\begin{figure}
\centerline{\includegraphics[width=9cm]{memory.png}}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.6\textwidth]{years.png}
...
\begin{figure}[!ht]
\centerline{\includegraphics[width=5cm]{papertype.png}}
...
\begin{figure}[!ht]
\centerline{\includegraphics[width=5cm]{algorithmtype.png}}
...
\begin{figure}[!ht]
\centerline{\includegraphics[width=5cm]{realproblem.png}}
