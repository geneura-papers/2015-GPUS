...
\runninghead{G. Romero et al.}

\title{Parallel Computation of Evolutionary Algorithms on GPUs. A review.} 

\author{Gustavo Romero\affilnum{1}, M. G. Arenas\affilnum{1}, Pedro Castillo\affilnum{1}, Antonio M. Mora\affilnum{1}, Pablo Garc\'ia-S\'anchez\affilnum{1} and Juan-Juli\'an Merelo\affilnum{1}}
...
Metaheuristics and bioinspired methods have been recently adapted to
massively parallel computation devices such as the General Purpose
Graphics Processing Units (GPGPUs). One of the most prolific
techniques taking advantage of this approach are the Evolutionary
Algorithms (EAs), since they have an inherently parallel structure,
based on a population of independent agents. 
This paper presents an exhaustive survey on all the literature related
to parallelization of EAs on GPUs. 
We have reviewed more than 200 publications, extracting their
features, such as the type of EA implemented, its properties, or if it
is applied to solve real world problems. Thus, we have defined a taxonomy of
the different approaches used in the parallelization
process. Moreover, a deeper analysis has been conducted on a big
selection, also describing the most representative in each category. 
The analysis shows that this is a very prolific research area, with an
increasing number of published papers every year. Genetic Algorithms
are so far the most widely used approach in EAs, and they are mainly applied
to solve real world problems due to the advantages that GPGPU computation
offers. 
% Antonio - TODO: incluir aquí alguna conclusión
%The paper also presents a review of hardware and software advances in these trends.
The paper also presents a review of advances in hardware and software along with trends in the field.
\end{abstract}
...
EAs present a structure which is inherently parallel, because the individuals of the population are relatively independent (they work independently, but all of them cooperate to find the solution). Thus, their distribution on different computation nodes is quite straight and not very difficult to implement, being normally a node devoted to work on subpopulations (island model) \cite{Cantu-Paz_parallel_GAs} and even, in the finest-grained models (or fully distributed), every node is assigned to one individual \cite{Alba02Parallelism,Tomassini_fully_distributed_EAs}.

The aim of the parallelization is usually the improvement of the running time for yielding an objective solution (if known), or for obtaining a certain degree of quality in the solution, if it is not the optimal. Sometimes this distribution could also imply a different searching scheme, which can lead to a
different searching area of the space of solutions and thus, to a different solution (or set of solutions) for an optimization problem, which, in turn, could be better than the one obtained in a sequential approach.
...
higher level programming languages used to take advantage on GPUs is described. The folloing section proposes a taxonomy based on the different parallel structures implemented in the literature, in order to categorize all the papers.
...
\citep{Buck:2004:BGS:1015706.1015800}.  % Esto deberÃ­a ir en la intro - JJ
...
\begin{figure*}[!ht]
\centering
\includegraphics[width=\textwidth]{cpu-gpu.png}
...
\begin{figure}[!ht]
\centerline{\epsfig{file=./hierarchy.eps,width=0.5\textwidth}}
...
\begin{figure}[!ht]
\centerline{\epsfig{file=grid.eps,width=0.50\textwidth}}
...
\begin{figure}
\centerline{\includegraphics[width=0.66\textwidth]{memory.png}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Classifying Evolutionary Algorithms}
\label{sec:taxonomy}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

This section proposes a taxonomy to distinguish between all possible methods used in the literature. Many different bioinspired approaches included in the bibliography, mainly focused on Parallel Evolutionary Computation (EC), because that is the most extended metaheuristic to address many types of large or complex problems, which require some kind of parallelization to be solved.with enough quality and on a reasonable quantity of time.
...
Alba classifies the main parallel metaheuristic models as follow: % si
                                % usÃ¡is bioinspired y metaheuristics
...
\subsection{Taxonomy}
This taxonomy is based on the classical approach followed by other authors, but it adds two new groups, hierarchical and non-standard models, because they appear with relative frequency.
...
\subsection{Collection technique}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.66\textwidth]{years.png}
\caption{Papers published per year inside the scope analysed here (EAs on GPGPUs) according to Web of Science database.}
\label{figure:publications}
\end{figure}

% Antonio - TODO: *** elegir una de las figuras ***
% Antonio - ¿estaban limitados los datos hasta 2004?¿Se hizo la consulta en 2015, porque hay un bajón ese año? Si es que sí, nos podrían decir que por qué no está 2016 y si es que no, podría deberse a que el tema está ya muy explotado y va siendo menos fructífero... habría que comentarlo, de ser así. ;)

With respect to the type of publication, it can be seen in Figure \ref{figure:type_publication} that conferences and journals share almost all the 
However, this factor should be taken as a reference, since not all the conferences and journals are indexed in WoS, but mainly relevant ones.

\begin{figure}[!ht]
\centerline{\includegraphics[width=0.4\textwidth]{papertype.png}}
\caption{Type of publication, being Conference paper, Journal Paper, or Book Chapter.}
\label{figure:type_publication}
\end{figure}

Regarding the type of EA implemented or studied in those papers, Figure \ref{figure:type_algorithm} shows the percentages of the different approaches. As expected, the most prolific ones are GAs, since it is the classic optimization method inside EAs. It is easy to implement and normally yields very good results.  GP and DE (Differential Evolution) are more specific and thus, less extended or used. `Others' refers to EAs such as EDA (Estimation of Distribution Algorithm), SGS (Systolic Genetic Search) or MA (Memetic Algorithms).

\begin{figure}[!ht]
\centerline{\includegraphics[width=0.4\textwidth]{algorithmtype.png}}
\caption{Type of Evolutionary Algorithm implemented in the analyzed publications.}
\label{figure:type_algorithm}
\end{figure}

Finally, the majority of the analysed publications deal with solving real problems as it is presented in Figure \ref{figure:type_problem}. This means that the parallelization by means of GPUs is very effective and can be used in relevant problems.

\begin{figure}[!ht]
\centerline{\includegraphics[width=0.33\textwidth]{realproblem.png}}
\caption{Portion of the analyzed publications which address real problems.}
\label{figure:type_problem}
\end{figure}


% Antonio - TODO: concluir ago y presentar las secciones siguientes... ¿¿¿en ellas se van a presentar los artículos más relevantes dentro de cada tipo en la taxonomía???
...
% maribel: he reescrito el pÃƒÂ¡rrafo siguiente entero, hay que revisarlo bien.
...
evaluates one individual. The programmer has to minimise the number of
global memory accesses, as well as to minimise data transfers between
...
such as a distance matrix or a dataset. This additional information
might be copied from CPU to GPU once at the beginning of the process,
so the threads might access to this data for evaluation minimising the
data transfers CPU-GPU.  

One of the first proposals for parallel Evolutionary Computation using
a GPU was done by Wong et
al. \citep{man-leung-wong-parallel-2005}. They presented an
...
\begin{acks}
This work has been partially funded by projects TIN2014-56494-C4-3-P (Spanish Ministry of Economy and Competitivity), PROY-PP2015-06 (Plan Propio 2015 UGR), and MSTR (PRY142/14, Fundaci\'on P\'ublica Andaluza Centro de Estudios Andaluces en la IX Convocatoria de Proyectos de Investigaci\'on).
\end{acks}
